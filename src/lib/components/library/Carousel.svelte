<script lang="ts">
	...
	export let dispatchFileActions: (action: string, file: any, currentIndex?: number) => void;

	type Components = {
		DcmDisplay: typeof DcmDisplay;
		ImageDisplay: typeof ImageDisplay;
		IFrameDisplay: typeof IFrameDisplay;
	};
	...
	let ViewerType: keyof Components | undefined;
	let fileBlob: Blob;
...
{:else}
	{#if previewPayload.type === 'file'}
		<Button on:click={loadPrevious} class="top-4 left-4 absolute z-40" variant="ghost"
			>Load Previous</Button
		>

		<Button on:click={loadNext} class="top-4 right-10 absolute z-40" variant="ghost"
			>Load Next</Button
		>
	{/if}

	<svelte:component this={components[ViewerType]} fileType={fileExtension} blob={fileBlob} />
{/if}
